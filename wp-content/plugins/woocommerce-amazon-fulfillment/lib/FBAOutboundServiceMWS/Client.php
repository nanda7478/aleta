...
class FBAOutboundServiceMWS_Client implements FBAOutboundServiceMWS_Interface
{
    ...
    {
    	// modified by Never Settle to support PHP 5.6 and up where iconv_set_encoding has been deprecated
   ...
   private function collapseWhitespace($s) {
	   // 3/21/2018 modified by Never Settle (AL) because we don't want any spaces
    ...
    private function quoteApplicationName($s) {
	    ...
	    // 3/21/2018 modified by Never Settle (AL) to remove these replacements because this is dumb and
	    // we already control exact UA anyway.
    ...
    private function _invoke(array $parameters)
    {
        try {
            ...
            for (;;) {
                ...
                // 3/11/2016 modified by Never Settle (AL) to fix bug... $response['Status'] is of type string
				// original will never work and will always throw an exception (same with 500):
                //if ($status === 200) {
                if ($status === '200') {
