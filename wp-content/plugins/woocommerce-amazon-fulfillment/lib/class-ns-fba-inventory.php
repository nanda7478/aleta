				...
				foreach ( $items as $sku => $stock ) {
					...
					foreach ( $products as $product ) {
                        ...
                        if ( $current_product 
							...
							// Test for WPML and if active, then use the TRID to find and loop through all translated products as well
							// setup global to support WPML with multiple translated IDs per product
							if ( function_exists( 'icl_object_id' ) ) {
					            ...
					            if ( is_numeric( $trid ) ) {
					                ...
					                if ( is_array( $translations ) ) {
					                    ...
					                    foreach ( $translations as $translation ) {
					                        ...
					                        if ( $this->ns_fba->wc->is_woo_version( '3.0' ) ) {
				                        		wc_update_product_stock( $trans_product, $stock );
				                        	} else {
				                            	$trans_product->set_stock( $stock );
											}
											error_log( $i . '-' . $translation->language_code . ': >> Updated translated ' . $sku . ' stock to ' . $stock . "<br />\n", 3, $this->ns_fba->inv_log_path );
					                    }
					                }
					            }						
							}
