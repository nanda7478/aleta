...
if ( ! class_exists( 'NS_FBA_Outbound' ) ) {
	...
	class NS_FBA_Outbound {
		...
		function test_api() {
			...
			$request->setQueryStartDateTime( date( 'c', time() ) );
		...
		function create_fulfillment_address( $order_id ) {
			// set initial address parameters and a copy to compare coverted versus original (raw)
			$shipping_name = $shipping_name_raw = get_post_meta( $order_id, '_shipping_first_name', true ) .
			                                      ' ' . get_post_meta( $order_id, '_shipping_last_name', true );
			$company_name = $company_name_raw = get_post_meta( $order_id, '_shipping_company', true );
			$address_line1 = $address_line1_raw = get_post_meta( $order_id, '_shipping_address_1', true );
			$address_line2 = $address_line2_raw = get_post_meta( $order_id, '_shipping_address_2', true );
			$address_city = $address_city_raw = get_post_meta( $order_id, '_shipping_city', true );
			$address_state = $address_state_raw = get_post_meta( $order_id, '_shipping_state', true );
		...
		function send_fulfillment_order( $order_id, $is_manual_send = false ) {
			try {
				...
				foreach ( $order_items as $item ) {
					$total_item_count++;
					// use WooCommerce to get either a normal or variation product from the item as a WC_Product
				...
				if ( ! $is_any_order_item_fba ) {
					...
					$request->setSellerId( NS_MERCHANT_ID );
					$request->setMarketplace( NS_MARKETPLACE_ID );
					$request->MarketplaceId = NS_MARKETPLACE_ID;
					...
					// -----------------------------------------------------------------------------------------------------------------
					// determing shipping speed from mapped settings or fall through to default
					// -----------------------------------------------------------------------------------------------------------------
					if ( $this->ns_fba->options['ns_fba_shipping_speed_priority'] != '' &&
					     $this->ns_fba->options['ns_fba_shipping_speed_priority'] == $order_shipping_method ) {
						...
						$shipping_speed_to_fba = 'Priority';
					           ...
					           $this->ns_fba->options['ns_fba_shipping_speed_expedited'] == $order_shipping_method ) {
						...
						$shipping_speed_to_fba = 'Expedited';
					           ...
					           $this->ns_fba->options['ns_fba_shipping_speed_standard'] == $order_shipping_method ) {
					...
					if ( isset( $this->ns_fba->options['ns_fba_update_inventory'] ) && '' != $this->ns_fba->options['ns_fba_update_inventory'] ) {
						...
						foreach ( $fba_skus as $fba_sku ) {
							...
							if ( '' == $inventory['message'] ) {
								...
								if ( $this->ns_fba->wc->is_woo_version( '3.0' ) ) {
	                        		wc_update_product_stock( $woo_product, $inventory['number'] );
	                        	} else {
	                            	$woo_product->set_stock( $inventory['number'] );
								}
		...
		function get_fulfillment_order_shipping_info( $order_id ) {
			...
			try {
				...
				$results = '<h2>Order Tracking Info</h2>' . $results;
...
break;
		...
		function set_order_status_safe( $order_id, $status ) {
			// $status is plain and this function will add wc-
			// NOTE: we cannot use wp_update_post or $order->update_status because they can trigger
			// an infinite loop especially when called from the Order Actions metabox action;
			// so instead, we use $wpdb->query for >= 2.2 and wp_set_post_terms for < 2.2
			if ( $this->ns_fba->wc->is_woo_version( '2.2.0' ) ) {
				global $wpdb;
				// and for the query we have to use the wc- status prefix since we are writing directly
				$ns_wc_term = 'wc-' . $status;
				$wpdb->query( $wpdb->prepare(
					'UPDATE ' . $wpdb->prefix . "posts SET post_status='%s' WHERE ID = %d",
					array( $ns_wc_term, $order_id )
				));
			} else {
				wp_set_post_terms( $order_id, $ns_wc_term, 'shop_order_status', false );
			}
		}
