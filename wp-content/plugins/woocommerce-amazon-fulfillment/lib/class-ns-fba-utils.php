...
if ( ! class_exists( 'NS_FBA_Utils' ) ) {
	...
	class NS_FBA_Utils {
		...
		function is_configured() {
			...
			if ( isset( $this->ns_fba->options['ns_fba_aws_access_key_id'] ) &&
				...
				if ( ! defined( 'NS_APPLICATION_NAME' ) ) {
					...
					$app_name = 'WooCommerceMCF/' . $this->ns_fba->version;
					define( 'NS_APPLICATION_NAME', $app_name );
				}
				if ( ! defined( 'NS_APPLICATION_VERSION' ) ) {
					// And we use the constant NS_APPLICATION_VERSION = options['ns_fba_app_name'] if set by user
					// so that the full value of the first part of the UA passed with requests to Amazon is:
					// "WooCommerceMCF/[version]/[user_app_name]"
					if ( isset( $this->ns_fba->options['ns_fba_app_name'] ) &&
						 '' !== $this->ns_fba->options['ns_fba_app_name'] ) {
						define( 'NS_APPLICATION_VERSION', $this->ns_fba->options['ns_fba_app_name'] );
					}
				}
		...
		function is_order_amazon_fulfill( $order, $is_manual_send ) {
			...
			if ( count( $excluded_shipping_options ) > 0 ) {
