		...
		class NS_FBA {
			...
			*/
			...
			private function __construct() {
				// debugging output rendered - change to false to disable debugging output
				// however, that might also disable logging output
				Kint::enabled( true );
				...
				// if a manual sync has been requested we need to wire that in
				// and it must be after woocommerce loaded of the wc_get_product dependency
				if ( isset( $_POST['ns_fba_sync_inventory_manually'] ) ) {
					add_action( 'wp_loaded', array( $this, 'handle_manual_sync' ), 11, 1 );
				}
			...
			/**
			 * Handle the POST request to manually syn all inventory levels
			 */
			function handle_manual_sync() {
				$this->inventory->sync_all_inventory();
				$text = sprintf( __( '<div class="updated"><p>Inventory Sync Complete! <a href="%s" target="_blank">Click here to see the results</a></p></div>', $this->text_domain ), $this->inv_log_url );
				echo $text;
			}
