...
class WC_Integration_FBA extends WC_Integration {
	...
	public function __construct() {
		...
		if ( isset( $_POST['ns_fba_clean_logs_now'] ) ) {
			add_action( 'admin_notices', array( $this, 'ns_fba_delete_logs_clicked' ) );
		}
	...
	function ns_fba_inventory_test_results() {
		...
		if ( -1 != $inventory['number'] ) {
			$text = sprintf( __( '<div class="updated"><p>Inventory Test Success! There are %1$s units of %2$s in FBA stock.</p></div>', $this->text_domain ), $inventory['number'], $this->nsfba->options['ns_fba_test_inventory_sku'] );
			echo $text;
		} else {
	 ...
	 * Delete Logs
	 */
	function ns_fba_delete_logs_clicked() {
		$files_deleted = $this->nsfba->utils->delete_older_logs();
		if ( 0 < $files_deleted ) {
			$text = sprintf( __( '<div class="updated"><p>%s log files deleted successfully.</p></div>', $this->text_domain ), $files_deleted );
			echo $text;
		} else {
			$text = sprintf( __( '<div class="error"><p>There were no files older than 30 days, or there was an error trying to delete them.</p></div>', $this->text_domain ) );
			echo $text;
		}
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'ns_fba_title1' => array(
				'title' => __( 'Amazon Account and MWS Settings', $this->text_domain ),
				'type' => 'title',
				'desc' => '',
			),
			...
			'ns_fba_app_name' => array(
				...
				'description'       => __( 'Used for MWS-required user-agent header. It can be anything, but no spacess like: MyStore.', $this->text_domain ),
				'desc_tip'          => true,
				'default'           => '',
				'type'              => 'text',
			),
